...
static void log_info_about_stored_lookup_tables(
    ...
    utils::g_log << "Stored lookup tables: " << num_stored_lookup_tables << "/"
                 << num_lookup_tables << " = "
                 << num_stored_lookup_tables /
                        static_cast<double>(num_lookup_tables)
                 << endl;
...
MaxCostPartitioningHeuristic::MaxCostPartitioningHeuristic(
    ...
    // We only need abstraction functions during search and no transition
    // systems.
...
int MaxCostPartitioningHeuristic::compute_heuristic(
    ...
    vector<int> abstract_state_ids =
        get_abstract_state_ids(abstraction_functions, state);
...
void MaxCostPartitioningHeuristic::print_statistics() const {
    ...
    int num_probably_superfluous =
        count(num_best_order.begin(), num_best_order.end(), 0);
    int num_probably_useful = num_orders - num_probably_superfluous;
    cout << "Number of times each order was the best order: " << num_best_order
         << endl;
    cout << "Probably useful orders: " << num_probably_useful << "/"
         << num_orders << " = " << 100. * num_probably_useful / num_orders
         << "%" << endl;
}
}
